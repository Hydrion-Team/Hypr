...
export class Logger implements ILogger {
	...
	private static readonly LOG_METHODS: ReadonlyMap<LogLevel, keyof Console> = new Map([
	...
	private static readonly DEFAULT_COLORS: ReadonlyMap<LogLevel, Color> = new Map([
	...
	private static readonly DEFAULT_NAMES: ReadonlyMap<LogLevel, string> = new Map([
		[LogLevel.Trace, 'TRACE'],
		[LogLevel.Debug, 'DEBUG'],
		[LogLevel.Info, 'INFO'],
		[LogLevel.Warn, 'WARN'],
	...
	protected preprocess(values: readonly unknown[]): string {
		const inspectOptions: InspectOptions = { colors: colorette.isColorSupported, depth: this.depth };
		return values.map(value => (typeof value === 'string' ? value : inspect(value, inspectOptions))).join(this.join);
	}

	private createFormatMap(options: LoggerFormatOptions = {}, defaults: LoggerLevelOptions): Map<LogLevel, LoggerLevel> {
	...
	private createDefaultLevel(defaults: LoggerLevelOptions, color: Color, name: string): LoggerLevel {
...
export class LoggerStyle {
	...
	private combineStyles(styles: Color[]): Color {
		...
		return (text: string) => styles.reduce((result, style) => style(result), text);
	}
}
